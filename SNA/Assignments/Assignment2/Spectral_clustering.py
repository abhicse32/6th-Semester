...
def create_degree_matrix(degree_count):
    D=[[[0] for val in range(419)] for vals in range(419)]
    for nodes in degree_count:
        D[nodes][nodes]=degree_count[nodes]
    return D
...
D=create_degree_matrix(degree_count)
L=nx.normalized_laplacian_matrix(G)
top_k=top_k_eigen_vectors(L.A)
Z=find_clusters(top_k)
print purity_(Z,labels)
#L=np.subtract(np.array(D),np.array(A))